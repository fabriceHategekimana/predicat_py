from modules.union import union, d, getVariables, ENTETE
...
def addFact(fact):
    ...
    if (
        fact.find(" ") == -1
    ):  # Si c'est pas une chaine de caractère (s'il y a des espaces)
        propagation(fact)
...
def propagation(fact):
    ...
    for i in range(3):
        fact[i] = "%" + fact[i] + "%"
    rules = d.sqlQuery(
        "select * from rules where premises like '%s' or premises like '%s' or premises like '%s'"
        % tuple(fact)
    )
    for rule in rules:
