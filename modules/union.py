...
def variableIndex(command):
    ...
    for i in range(len(tab)):  # on explore les éléments de la commande
        if tab[i] in [
            "not",
            "",
        ]:  # si on tombe sur un 'not' ou pas on l'ajoute par défaut
            values.append(tab[i])
        elif isVariable(tab[i]):  # si on trouve une variable
            variables.append(
                column[i] + " as " + tab[i]
            )  # on l'ajoute à la table des variables
...
def convert(exp):
    ...
    if isFilter(exp):  # if it's an expression with a comparison operation
        ...
        t = tuple(exp)
        sql = (
            "(select * from facts where %s subject='%s' and %s link='%s' and %s goal='%s'"
            % t
            + ")"
        )
    else:
